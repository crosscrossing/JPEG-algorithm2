	...
	case 17: {
		...
		if (sof.numberOfColor == 3) {
...
#ifdef ENCRYPT
			{
				...
				decryptComputeDiff(cbDeQT, numMCU);
				decryptComputeDiff(crDeQT, numMCU);
			...
			/*	}*/
				...
				decryptReComputeDiff(yDeQT, numMCU);
				decryptReComputeDiff(cbDeQT, numMCU);
...
int JPEGDecodeDecrypt::generateEncryptedHuffmanTable(DHT dht, BYTE * huffLength, WORD * huffCode, BYTE * huffValue, DEHUFFEncrypted * dehuffEncrypt) {
	...
	//哈夫曼mutate
	for (int i = 1; i <= sum; i++) {
		BYTE byte = ((long long)logistic()*1.0e+15) % 256;
...
void JPEGDecodeDecrypt::decrypt(SWORD(*yMcu)[64], SWORD(*cbMcu)[64], SWORD(*crMcu)[64], int numMCU) {
	...
	for (int round = 0; round < ROUND; round++) {
		...
		thread thYDC(decryptDCFunc, mcu, numMCU, std::ref(lvYDC), std::ref(logisticYDC));
		thread thCDC(decryptDCFunc, mcu + numMCU, 2 * numMCU, std::ref(lvCDC), std::ref(logisticCDC));

		//ac decrypt
		HyperChaoticLvSystem lvYAC(x, y, z, u);
		LogisticMap logisticYAC(lx);
		decryptAC(mcu, 3 * numMCU, lvYAC, logisticYAC);
		//recover ac block scramble 
		HyperChaoticLvSystem lvBlockY(x, y, z, u);
		HyperChaoticLvSystem lvBlockC(lvBlockY);
		decryptBlockScrambling(mcu, numMCU, lvBlockY);
...
void JPEGDecodeDecrypt::decryptComputeDiff(SWORD(*mcu)[64], int numMCU) {
	//计算差分值
	int pred = mcu[0][0];
...
void JPEGDecodeDecrypt::decryptReComputeDiff(SWORD(*mcu)[64], int numMCU) {
	//差分值还原
	int pred = mcu[0][0];
	for (int i = 1; i < numMCU; i++) {
		pred = mcu[i][0] + pred;
