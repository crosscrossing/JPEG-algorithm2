...
#ifdef ENCRYPT
	{
		encryptComputeDiff(yResult, numMCU);
		encryptComputeDiff(cbResult, numMCU);
		encryptComputeDiff(crResult, numMCU);

		clock_t begin = clock();
	/*	string posFileName = fileName.erase(fileName.length() - 4, 4);
		posFileName += ".txt";
		...
		if (posFile.good() && objectFile.good()) {
			...
			cout << "No position file or object file. Fully encrypting instead." << endl;
			encrypt(yResult, cbResult, crResult, numMCU);
	/*	}*/
		cout << "Encryption cost time " << (double)(clock() - begin) / CLOCKS_PER_SEC << 's' << endl;
		//encryptReComputeDiff(yResult, numMCU);
		//encryptReComputeDiff(cbResult, numMCU);
		//encryptReComputeDiff(crResult, numMCU);
		//编码时diff已注释
	}
#endif
...
int JPEGEncode::writeDC(ENHUFF*enHuff, SWORD*afterQT, SWORD&pred) {
	...
	if (diff == 0) {
		...
		if (diff > 2047 || diff < -2047)cout << "overflow";
...
int JPEGEncodeEncrypt::writeDC(ENHUFF*enHuff, SWORD*afterQT, SWORD&pred) {
	SWORD diff = afterQT[0]/* - pred*/;
	BYTE digit;//待编码值位数
	SWORD temp;//用来判断位数是多少的临时变量
	WORD nagative_temp;//如果是负数进行调整
...
void JPEGEncodeEncrypt::encrypt(SWORD(*yMcu)[64], SWORD(*cbMcu)[64], SWORD(*crMcu)[64], int numMCU) {
	...
	// 抗差分部分：生成差分图像代码
	for (int i = 0; i < numMCU; i++) {
		if (mcu[i][0] != 0 && mcu[i][0] != 1 && mcu[i][0] != -1) {
			mcu[i][0] = -mcu[i][0] ^ 1;
			break;
		}
	}
	for (int i = numMCU; i < numMCU * 3; i++) {
		if (mcu[i][0] != 0 && mcu[i][0] != 1 && mcu[i][0] != -1) {
			mcu[i][0] = -mcu[i][0] ^ 1;
			break;
		}
	}
	for (int i = 0; i < numMCU * 3; i++) {
		bool exit = false;
		for (int j = 1; j < 64; j++) {
			if (mcu[i][j] != 0 && mcu[i][j] != 1 && mcu[i][j] != -1) {
				mcu[i][j] = -mcu[i][j] ^ 1;
				exit = true;
				break;
			}
	...
	*/
	...
	for (int round = 0; round < ROUND; round++) {
		//dc encrypt
		HyperChaoticLvSystem lvYDC(x, y, z, u), lvCDC(lvYDC);
		LogisticMap logisticYDC(lx), logisticCDC(logisticYDC);
		...
		thread thYDC(encryptDCFunc, mcu, numMCU, std::ref(lvYDC), std::ref(logisticYDC));
		thread thCDC(encryptDCFunc, mcu + numMCU, 2 * numMCU, std::ref(lvCDC), std::ref(logisticCDC));
		//ac encrypt
		//ac block scrambling
		HyperChaoticLvSystem lvBlockY(x, y, z, u);
		HyperChaoticLvSystem lvBlockC(lvBlockY);
		encryptBlockScrambling(mcu, numMCU, lvBlockY);
...
void JPEGEncodeEncrypt::encryptBlockScrambling(SWORD(*mcu)[64], int numMCU, ChaoticMap &chaoticMap) {
	...
	for (int i = 0; i < numMCU; i++) {
		...
		memcpy(dataTemp, mcuPointer + 1, sizeof(SWORD) * 63);
		memcpy(mcuPointer + 1, mcuPointer + 64 * (*keyStreamPointer) + 1, sizeof(SWORD) * 63);
		memcpy(mcuPointer + 64 * (*keyStreamPointer) + 1, dataTemp, sizeof(SWORD) * 63);
